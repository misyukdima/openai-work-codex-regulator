#!/usr/bin/env python3
"""Build a portable release ZIP and validate it with a clean round-trip."""
from pathlib import Path
import subprocess
import sys
import tempfile
import zipfile

ROOT=Path(__file__).resolve().parents[1]
SKILL_DIR="openai-work-codex-regulator"
VALIDATORS=("scripts/validate_repo_v4.py","scripts/validate_v4_routing.py","scripts/validate_plugin_package.py","scripts/validate_release_contract.py")
EXCLUDE_DIRS={".git","__pycache__","dist",".idea",".vscode"}
EXCLUDE_FILES={".DS_Store"}

def fail(msg):
    print(f"release validation FAILED: {msg}")
    raise SystemExit(1)

def collect_files(root):
    files=[]
    for path in sorted(root.rglob("*")):
        if not path.is_file(): continue
        rel=path.relative_to(root)
        if any(part in EXCLUDE_DIRS for part in rel.parts): continue
        if path.name in EXCLUDE_FILES or path.suffix==".zip" or path.name.startswith(".env"): continue
        if not rel.as_posix().isascii(): fail(f"non-ASCII filename: {rel.as_posix()}")
        files.append(rel)
    return files

def run_validators(root,label):
    for rel in VALIDATORS:
        path=root/rel
        if not path.is_file(): fail(f"{label}: missing validator {rel}")
        r=subprocess.run([sys.executable,str(path)],capture_output=True,text=True,cwd=root)
        if r.stdout.strip(): print(r.stdout.strip())
        if r.returncode:
            if r.stderr.strip(): print(r.stderr.strip())
            fail(f"{label}: {rel}")

def main():
    version=(ROOT/"VERSION").read_text().strip()
    run_validators(ROOT,"source tree")
    files=collect_files(ROOT)
    dist=ROOT/"dist"; dist.mkdir(exist_ok=True)
    zip_path=dist/f"{SKILL_DIR}-v{version}.zip"
    with zipfile.ZipFile(zip_path,"w",zipfile.ZIP_DEFLATED) as zf:
        for rel in files: zf.write(ROOT/rel,f"{SKILL_DIR}/{rel.as_posix()}")
    with tempfile.TemporaryDirectory(prefix="openai-regulator-release-") as tmp:
        with zipfile.ZipFile(zip_path) as zf: zf.extractall(tmp)
        unpacked=Path(tmp)/SKILL_DIR
        actual=sorted(p.relative_to(unpacked).as_posix() for p in unpacked.rglob("*") if p.is_file())
        expected=sorted(rel.as_posix() for rel in files)
        if actual!=expected: fail("round-trip file mismatch")
        run_validators(unpacked,"unpacked artifact")
    print(f"release artifact OK — {zip_path.relative_to(ROOT)} passed clean ZIP round-trip")

if __name__=="__main__":
    main()
